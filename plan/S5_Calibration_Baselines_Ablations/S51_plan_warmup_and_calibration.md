...
## Definition of Done
- [ ] **Acceptance command:** `.venv/bin/python -m pytest tests/test_calibration.py -q` (and `.venv/bin/python -m src.main --mode warmup` to produce the frozen artifacts).
- [ ] **Tests:** offline & deterministic (`Config(offline=True)`, `MockLLM`, fixtures — no network/AV/real LLM). Assert isotonic **monotonicity** (`predict_proba` non-decreasing in `z`) and a **reliability check on synthetic data** (a known mis-scaled `z` is bent toward the diagonal; Brier/ECE improve post-fit). The **"hit" label is the sign of the drift-demeaned forward return matching the action** — identical rule to the memory reward (`_hit_label` uses `h` + `reward_drift_window`, μ point-in-time `≤ t`).
- [ ] **Gate:** `make check` green (lint + typecheck + test + e2e).
- [ ] **features.json:** `F15` → `passing` with evidence (the passing `test_calibration.py` command + date, reliability diagram path).
- [ ] **Artifacts:** `results/calibrator.pkl`, `results/reliability_diagram.png`, `results/calibration_report.json` (method, n, per-bin counts, Brier, ECE); diagnostic `results/warmup_pairs.csv`. Warm-up FAISS index persisted under `data/`/`faiss_index/` (gitignored).
- [ ] **Rules:** calibrate **ONLY on 2022–2024 warm-up** and **FREEZE** `calibrator.pkl` before any test-window run; **NEVER report warm-up PnL** (no PnL artifact emitted from warm-up — only a warmed memory + frozen calibrator); reuse the LLM cache; all thresholds/windows are numbers in `config.py`, never hardcoded.
- [ ] **Tracking:** `PROGRESS.md` updated; `DECISIONS.md` ADR for the isotonic-vs-Platt fallback choice (small-data threshold) if non-obvious.
