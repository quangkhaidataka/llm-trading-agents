...
def test_load_news_slices_to_t(monkeypatch) -> None:
    ...
    # Fixture AAPL items: 2024-05-31, 2024-06-02, 2024-06-03 (all relevance >= 0.3).
    assert len(loaders.load_news("AAPL", date(2024, 6, 3))) == 3
    # Only the 2024-05-31 item is <= 2024-06-01.
...
def test_load_news_relevance_filter(monkeypatch) -> None:
    ...
    # relevances 0.92 / 0.68 / 0.74 → cutoff 0.7 keeps 0.92 and 0.74, drops 0.68.
...
def test_load_macro_no_relevance_filter_and_slice(monkeypatch) -> None:
    _use(Config(offline=True), monkeypatch)
    # Macro items: 2024-05-30, 2024-06-01, 2024-06-02 — none dropped by relevance.
    assert len(loaders.load_macro_news(("economy_macro",), date(2024, 6, 2))) == 3
    assert len(loaders.load_macro_news(("economy_macro",), date(2024, 5, 31))) == 1
